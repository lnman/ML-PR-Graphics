...
void find_result(string ans){
	...
	for (int i = 0; i < 15; ++i)
	{
		vp.push_back(make_pair(di[i/5][i%5],0));
	}
	...
	//k==1
	if(d[vp[0].second].topic==ans){
		acc[0]++;
	}
	if(d[vp[5].second].topic==ans){
		acc[1]++;
	}
	if(d[vp[10].second].topic==ans){
		acc[2]++;
	}
	//k==3
	if(majority(3,0)==ans){
...
void test(){
	...
	in.close();
	cout<<double(acc[0])/tot<<" "<<double(acc[1])/tot<<" "<<double(acc[2])/tot<<endl;
	cout<<double(acc[3])/tot<<" "<<double(acc[4])/tot<<" "<<double(acc[5])/tot<<endl;
	cout<<double(acc[6])/tot<<" "<<double(acc[7])/tot<<" "<<double(acc[8])/tot<<endl;
}
